...
import (
	...
	"github.com/victordev13/fclx-chat/chatservice/internal/domain/entity"
	"github.com/victordev13/fclx-chat/chatservice/internal/domain/gateway"
)
...
func NewChatCompletionUseCase(chatGateway gateway.ChatGateway,
	openAiClient *openai.Client,
	stream chan ChatCompletionOutputDTO,
...
func (uc *ChatCompletionUseCase) Execute(
	ctx context.Context,
	...
	aiResponse, err := uc.OpenAiClient.CreateChatCompletionStream(ctx, openai.ChatCompletionRequest{
	...
	for {
		response, err := aiResponse.Recv()
		if errors.Is(err, io.EOF) { //if stream is ended, finish the request
			break
		}
